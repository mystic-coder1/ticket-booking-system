...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media (max-width: 640px) {
    ...
    /* Stack lookup inputs vertically */
    .lookup-row { 
      flex-direction: column; 
      gap: 12px; 
    }
    ...
    .ticket-strip { padding: 12px 16px; }
    .ticket-body { padding: 16px; }
    .ticket-route { padding: 12px 16px; }
    .route-city { font-size: 18px; }
  }
`;
...
function BookingForm({ onBooked }) {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/register-passenger`, {
        method: "POST",
...
function TicketView({ initialTicketNumber }) {
  ...
  const lookup = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/get-ticket?ticketNumber=${input}`);
